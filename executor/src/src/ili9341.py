...
class Display(object):
    ...
    def draw_image(self, path, x=0, y=0, w=320, h=240):
        ...
        if self.is_off_grid(x, y, x2, y2):
            #Returns: boolean: False = Coordinates OK, True = Error.
            return
        with open(path, "rb") as f:
            chunk_height = 1024 // w
            chunk_count, remainder = divmod(h, chunk_height)
            print('chunk_height,chunk_count, remainder',chunk_height,chunk_count, remainder)
    ...
    def is_off_grid(self, xmin, ymin, xmax, ymax):
        ...
        if ymin < 0:
            ...
            return True
        if self.rotation_num in [90, 270]:
            if xmax > self.height:
